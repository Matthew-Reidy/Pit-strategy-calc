...
namespace AssettoCorsaSharedMemory
{
    ...
    public class AssettoCorsa
    {
        ...
        private ACC_STATUS gameStatus = ACC_STATUS.AC_OFF;
        ...
        {
            { ACC_STATUS.AC_OFF, "Off" },
            { ACC_STATUS.AC_LIVE, "Live" },
            { ACC_STATUS.AC_PAUSE, "Pause" },
            { ACC_STATUS.AC_REPLAY, "Replay" },
        };

        public AssettoCorsa()
        {
            sharedMemoryRetryTimer = new Timer(2000);
            sharedMemoryRetryTimer.AutoReset = false;
        ...
        public Graphics ReadGraphics()
        {
            ...
            using (var stream = graphicsMMF.CreateViewStream())
            {
                using (var reader = new BinaryReader(stream))
                {
                    ...
                    var bytes = reader.ReadBytes(size);
                    var handle = GCHandle.Alloc(bytes, GCHandleType.Pinned);
                    var data = Marshal.PtrToStructure(handle.AddrOfPinnedObject(), typeof(Graphics));
                    handle.Free();
